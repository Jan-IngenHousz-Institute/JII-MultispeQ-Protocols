...
def generate_individual_module_rst():
  ...
  for _, module_name, _ in pkgutil.iter_modules(package.__path__):
    ...
    ## Extract header
    if hasattr( getattr(jii_multispeq_protocols, f"{module_name}"), "__doc__"):
      doc = getattr( getattr(jii_multispeq_protocols, f"{module_name}"), "__doc__").split('\n')
    ...
    if hasattr( getattr(jii_multispeq_protocols, f"{module_name}"), "_settings"):
      settings = getattr( getattr(jii_multispeq_protocols, f"{module_name}"), "_settings")
    ...
    if hasattr( getattr(jii_multispeq_protocols, f"{module_name}"), "_example"):
            
      _example = getattr( getattr(jii_multispeq_protocols, f"{module_name}"), "_example")
      ...
      if hasattr( getattr(jii_multispeq_protocols, f"{module_name}"), "_analyze"):
        _analyze = getattr( getattr(jii_multispeq_protocols, f"{module_name}"), "_analyze")
      else:
        def _analyze( _data ):
          return {}
      ...
      try:
        ...
        for key in keys:
          ...
          if value is None:
            ...
            table_content.append([key, value])
          elif isinstance(value, (list, np.ndarray )) and key not in  ["protocol", "settings", "order"]:
